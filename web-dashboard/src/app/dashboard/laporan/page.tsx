...
export default function LaporanKasPage() {
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-400 uppercase bg-slate-900/30 border-b border-white/10">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-4 font-semibold text-slate-300">Siswa</th>
                  <th scope="col" className="px-6 py-4 font-semibold text-slate-300">Keterangan</th>
                  <th scope="col" className="px-6 py-4 font-semibold text-slate-300 text-right">Nominal Masuk</th>
              ...
              <tbody className="divide-y divide-white/5 relative">
                {isLoading ? (
                  ...
                  <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{new Date(item.tanggal).toLocaleDateString('id-ID')}</td>
                    <td className="px-6 py-4">
                      <p className="font-medium text-white">{item.siswa}</p>
  ...
  return (
    ...
    >
      ...
      {/* Filter Card */}
      <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 border border-white/10 shadow-xl mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <button className="w-full py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-blue-300 font-semibold rounded-lg text-sm transition-colors shadow-sm">
              Terapkan Filter
            </button>
          </div>
        </div>
      ...
      <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden shadow-xl">
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-medium text-white">{item.keterangan}</td>
                  <td className="px-6 py-4 text-right font-semibold text-emerald-400 whitespace-nowrap">
                    + {formatRupiah(item.nominal)}
                  </td>
                </tr>
              ))
        ...
        <div className="p-4 border-t border-white/10 bg-slate-900/30 text-xs text-slate-400 flex justify-between items-center">
