...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-72 bg-navy-900 border-r border-white-10 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
        ...
        <div className="p-6 border-b border-white-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white-40">Dashboard SaaS</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {navItems.map((item) => {
            ...
            const activeClasses = "bg-gradient-to-r from-blue-500/15 to-transparent border-l-4 border-blue-400 text-blue-300 font-semibold";
        ...
        <div className="p-4 border-t border-white-10">
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-9 h-9 rounded-full bg-gold-400/20 flex items-center justify-center text-sm font-bold text-gold-400">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gold-400">{roleLabel}</p>
            </div>
          </div>
          <button
            onClick={logout}
            className="mt-2 w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm text-white-60 hover:bg-status-gagal-bg hover:text-status-gagal transition-all duration-200"
          >
            <span>🚪</span>
            <span>Keluar</span>
          </button>
